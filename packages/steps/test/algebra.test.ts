...
describe("explanations", () => {
  ...
  it("leaves no unfilled placeholder anywhere in the corpus", () => {
    // A placeholder whose value was never passed renders as nothing, so the
    // gap it leaves behind is what gives it away. Braces belonging to LaTeX
    // are dropped first, or every \text{or} would read as a placeholder.
    ...
    for (const p of problems) {
      ...
      for (const step of outcome.solution.steps) {
        ...
        expect(bare(step.title), where).not.toMatch(/\{[a-z]+\}/);
        expect(bare(step.explanation), where).not.toMatch(/\{[a-z]+\}/);
