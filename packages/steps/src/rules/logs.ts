...
export const combineLogs: Rule = {
  ...
  apply(n) {
    ...
    for (const term of n.args) {
      const direct = asLog(term);
      if (direct) {
        entries.push({ log: direct, negated: false, term });
        continue;
      }
      if (term.type === "neg") {
        const inner = asLog(term.arg);
        if (inner) {
...
export const logRules: Rule[] = [logPower, combineLogs, logOfPower];
