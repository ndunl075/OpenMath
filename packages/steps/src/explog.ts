...
import { asLogTerm } from "./rules/log-equations.js";
import { arithmeticRules, divideBothSides, identityRules, isolationRules, logEquationRules } from "./rules/index.js";
...
export function solveLogarithmicEquation(
  ...
  const combined = run(scaled.node, logEquationRules, { variable });
  ...
  const oriented = orient(isolated.node, steps, (side) => {
    const log = asLogTerm(side);
    return !!log && hasVariable(log.argument, variable);
  });
  if (!oriented) return null;
  const log = asLogTerm(oriented.lhs);
