...
5. **v1 launch** — blocked only on steps 1 and 2. The solver covers arithmetic, exact fractions, roots, expanding, like terms, linear equations, inequalities, quadratics, derivatives and integrals, verified against 178 corpus problems, seventeen of which must be refused rather than answered.
6. **v1.5**: method switcher, more rule coverage from contributors. **v2**: derivatives (TS) — **DONE**. **v3**: integrals — **DONE**, in TS rather than Pyodide + SymPy. The native rule engine that was **v4** landed first, in v1.
